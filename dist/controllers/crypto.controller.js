...
export async function generateQrHandler(request, reply) {
    if (!request.user || !request.user.businessId) {
        return reply.status(401).send({ error: 'Unauthorized' });
    }
    const { currency, amount, saleId } = request.body;
    if (!currency || !amount) {
...
export async function getWalletsHandler(request, reply) {
    if (!request.user || !request.user.businessId) {
        return reply.status(401).send({ error: 'Unauthorized' });
    }
...
export async function upsertWalletHandler(request, reply) {
    if (!request.user || !request.user.businessId) {
        return reply.status(401).send({ error: 'Unauthorized' });
    }
...
export async function confirmPaymentHandler(request, reply) {
    const { paymentId, txHash } = request.body;
    if (!paymentId || !txHash) {
...
export async function adminChangeStaffPinHandler(request, reply) {
    if (!request.user)
        return reply.status(401).send({ error: 'Unauthorized' });
