...
export class ForgotPinCommand
  implements ICommand<ForgotPinInput, { message: string }>
{
  async execute(input: ForgotPinInput) {
    const user = await userRepository.findByEmail(input.email)
    if (!user) return { message: 'Invalid or expired OTP' }

    const otp = await otpRepository.findValid(user.id, input.otp, 'pin_reset')
    if (!otp) return { message: 'Invalid or expired OTP' }
