...
export class AdminChangeStaffPinCommand
  implements ICommand<AdminChangeStaffPinInput, { message: string }>
{
  async execute(input: AdminChangeStaffPinInput) {
    const admin = await userRepository.findById(input.adminId)
    if (!admin) throw new Error('Admin not found')
    if (!admin.businessId) throw new Error('Admin has no business assigned')
